...
class VideoFACT(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if fe_ckpt is not None and len(fe_ckpt) > 0:
            self.pretrained_fe = self.pretrained_fe.load_from_checkpoint(fe_ckpt, **fe_config)
        ...
        self.classifier = nn.Sequential(
            nn.Conv2d(embed_dim, embed_dim, kernel_size=(4, 4), bias=True),
            nn.ReLU(),
            nn.BatchNorm2d(embed_dim),
            nn.MaxPool2d(kernel_size=(2, 2), stride=2),
            nn.Dropout(0.2),
            nn.Flatten(),
            nn.LazyLinear(1024*2),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.LazyLinear(1024),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        _, NUM_PATCH_HEIGHT, NUM_PATCH_WIDTH, _, _, _ = patches.shape
        ...
        # we need to go from BPC to BCP 
        # then to BC-NUM_PATCH_HEIGHT-NUM_PATCH_WIDTH to feed into the classifier
        # and to BCP1 in order to feed into the localizer with nn.Conv2d 1x1 layers (reducing C -> 1)
        scaled_bb_fe = scaled_bb_fe.permute(0, 2, 1)
        class_label = self.classifier(scaled_bb_fe.view(B, self.embed_dim, NUM_PATCH_HEIGHT, NUM_PATCH_WIDTH))
        patch_label = self.localizer(scaled_bb_fe.unsqueeze(-1))
